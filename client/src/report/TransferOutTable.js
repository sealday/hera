...
class TransferOutTable extends React.Component {
  ...
  query = condition => {
    this.props.dispatch(queryStore('调拨出库明细', {
      ...condition,
  ...
  render() {
    return (
      <div>
        <h2 className="page-header">调拨出库明细</h2>
        <SearchForm fieldname="入库" form="调拨出库明细" onSubmit={this.query}/>
        <Table name="入库" records={this.props.records}/>
      </div>
    );
  }
}
...
const mapStateToProps = state => {
  ...
  const records = state.results.get('调拨出库明细', [])

  return {
    records: records.filter(record => record.outStock === store._id && record.type === '调拨'),
    store,
  }
}
