...
export const PageHeaderComponent = ({
  ...
  if (searchForm) {
    const {
      ...
      onSubmit,
      onExcelExport,
      ...otherSearchFormProps
    } = searchForm
    actions.push(
      <Button
        key="onReset"
        onClick={() => mSearchForm.current.reset()}
        icon={<ClearOutlined />}
      >
        重置
      </Button>
        ...
        <>
          {search && searchMeta ? (
            <Form
              colon={false}
              form={form}
              onFinish={search.onSubmit}
              initialValues={searchMeta.initialValues}
            >
              {searchMeta.formItems}
            </Form>
          ) : (
            <></>
          )}
