  ...
  if (dataSource) {
    columns.forEach(column => {
      if (
        ...
        if (column.render) {
          column.filters = _.uniq(
            dataSource
              .map(record => column.render(record[column.dataIndex], record))
              .map(v => _.get(v, 'props.children', v))
          ).map(v => ({
            text: _.toString(v) ? v : '未填写',
            value: _.toString(v) ? v : '__未填写__',
          }))
        } else {
          column.filters = _.uniq(
            dataSource.map(record => record[column.dataIndex])
          ).map(v => ({
            text: _.toString(v) ? v : '未填写',
            value: _.toString(v) ? v : '__未填写__',
          }))
